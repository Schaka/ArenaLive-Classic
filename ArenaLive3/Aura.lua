...
if LibClassicDurations then
    LibClassicDurations:Register("ArenaLive")
    UnitAura = LibClassicDurations.UnitAuraWithBuffs
	LibClassicDurations.RegisterCallback("ArenaLive", "UNIT_BUFF", function(event, unit)
        Aura:OnEvent(event, unit)
    end)
end
...
function Aura:UpdateAuras(unitFrame, auraType)
	...
	while auraID <= NUM_MAX_AURAS do
		...
		-- Retrieve buff info:
		if ( auraType == "BUFF" ) then
		    name, texture, count, dispelType, duration, expires, caster, isStealable, _ , spellID, _, _, casterIsPlayer, nameplateShowAll = UnitAura(unit, auraID, "HELPFUL");
		elseif ( auraType == "DEBUFF" ) then
		    name, texture, count, dispelType, duration, expires, caster, isStealable, _, spellID, _, _, casterIsPlayer, nameplateShowAll = UnitAura(unit, auraID, "HARMFUL");
