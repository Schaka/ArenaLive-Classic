...
-- Test values:
CastBar.testValues = {2061, 5176, 8679, 7328, 118};
...
function CastBar:StartCast(castBar, unit, event)
	...
	local name, text, icon, startTime, endTime, isTradeSkill, castID, spellID, notInterruptible, value, maxValue;
	if ( event == "UNIT_SPELLCAST_START" ) then
		name, text, icon, startTime, endTime, isTradeSkill, castID, notInterruptible, spellID = UnitCastingInfo(unit);
	...
	elseif ( event == "UNIT_SPELLCAST_CHANNEL_START" ) then
		name, text, icon, startTime, endTime, isTradeSkill, castID, notInterruptible, spellID = UnitChannelInfo(unitID)
