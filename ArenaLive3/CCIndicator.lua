...
if LibClassicDurations then
	...
	LibClassicDurations.RegisterCallback("ArenaLive", "UNIT_BUFF", function(event, unit)
        CCIndicator:UpdateCache(event, unit)
    end)
end
...
function CCIndicator:UpdateCache (event, unit)
	...
	for i = 1, MAX_BUFFS, 1 do
		spellName, texture, _, _, duration, expires, _, _, _ , spellID, _, _, _, _ = UnitAura(unit, i, "HELPFUL");
	...
	for i = 1, MAX_DEBUFFS, 1 do
		spellName, texture, _, _, duration, expires, _, _, _, spellID, _, _, _, _ = UnitAura(unit, i, "HARMFUL");
