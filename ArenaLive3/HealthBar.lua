...
function healCommEventHandler(event, casterGUID, spellID, spellType, endTime, ...)
	if type(...) == "string" then
		for predictUnit, enabled in pairs(HealPredictionUnits) do
			if UnitGUID(predictUnit) == ... and enabled then
				HealthBar:OnEvent("UNIT_HEAL_PREDICTION", predictUnit, casterGUID, spellID);
			end	
		end
	elseif type(...) == "table" then 	
		for _, targetGuid in pairs(...) do
			for predictUnit, enabled in pairs(HealPredictionUnits) do
				if UnitGUID(predictUnit) == targetGuid and enabled then
					HealthBar:OnEvent("UNIT_HEAL_PREDICTION", predictUnit, casterGUID, spellID);
				end	
			end
		end
...
HealComm.RegisterCallback("ArenaLive", "HealComm_HealStarted", healCommEventHandler);
HealComm.RegisterCallback("ArenaLive", "HealComm_HealStopped", healCommEventHandler);
...
function HealthBar:UpdateHealPrediction (unitFrame)
	...
	local maxHealth = UnitHealthMax(unit);
	local currHealth = healthBar.currValue;
	local predictedHeal = HealComm:GetHealAmount(UnitGUID(unit), HealComm.ALL_HEALS, GetTime() + 5) or 0
	if ( maxHealth <= 0 ) then
		return;
	end
