...
local isTBC = build == 2 or WOW_PROJECT_ID and WOW_PROJECT_ID == WOW_PROJECT_BURNING_CRUSADE_CLASSIC
local isWrath = build == 3 or WOW_PROJECT_ID and WOW_PROJECT_ID == WOW_PROJECT_BURNING_CRUSADE_CLASSIC
...
if( playerClass == "DRUID" ) then
	LoadClassData = function()
		...
		local Lifebloom = GetSpellInfo(408124) or GetSpellInfo(33763) or "Lifebloom"
		local EmpoweredRejuv = GetSpellInfo(33886) or "Empowered Rejuv"
		local EmpoweredTouch = GetSpellInfo(33879) or "Empowered Touch"
		local WildGrowth = GetSpellInfo(408120) or GetSpellInfo(48438) or "Wild Growth"
		...
		elseif isClassic then
			...
			hotData[Lifebloom] = {interval = 1, ticks = 7, coeff = 0.357, dhCoeff = 0.274, levels = {10}, averages = {28}, bomb = {58}}
			hotData[WildGrowth] = {interval = 1, ticks = 7, coeff = 0.427, levels = {10}, averages = {35}}
		elseif isTBC then
			hotData[Rejuvenation] = { interval = 3, levels = { 4, 10, 16, 22, 28, 34, 40, 46, 52, 58, 60, 63, 69 }, averages = { 32, 56, 116, 180, 244, 304, 388, 488, 608, 756, 888, 932, 1060 }}
...
if( playerClass == "PRIEST" ) then
	LoadClassData = function()
		...
		local Penance = GetSpellInfo(402284) or GetSpellInfo(53007) or "Penance"
		...
		elseif isClassic then
			-- FIXME: correct coeff?
			spellData[Penance] = {_isChanneled = true, coeff = 0.857, ticks = 3, levels = {1}, averages = {318}}	
		end
...
if( playerClass == "SHAMAN" ) then
	LoadClassData = function()
		...
		local EarthShield = GetSpellInfo(408514) or GetSpellInfo(49284) or "Earth Shield"
...
function HealComm:PLAYER_TARGET_CHANGED()
	...
	-- only overwrite the unit if it's not set to something else already (i.e, we're in party and it's party3)
	if guid and not guidToUnit[guid] then
		guidToUnit[guid] = "target"
	end
end
