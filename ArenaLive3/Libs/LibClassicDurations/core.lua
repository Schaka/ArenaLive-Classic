...
local enableEnemyBuffTracking = true
...
function f:PLAYER_LOGIN()
    ...
    --LCD_SpellInfo = {}; -- FIXME WHEN STABLE
    if not LCD_SpellInfo then
        LCD_SpellInfo = {};
    end    
end
...
local function getToolTipInfo(tooltip)
    local spellName = _G[tooltip:GetName() .. "TextLeft1"]:GetText();
    local spellInfo = _G[tooltip:GetName() .. "TextLeft2"] and _G[tooltip:GetName() .. "TextLeft2"]:GetText() or "-";
    local buffType = _G[tooltip:GetName() .. "TextRight1"] and _G[tooltip:GetName() .. "TextRight1"]:GetText() or "-";
    local timeRemainingString = _G[tooltip:GetName() .. "TextLeft3"] and _G[tooltip:GetName() .. "TextLeft3"]:GetText() or nil;
    local timeLeft = nil;

    if spellName == "Supercharged Chronoboon Displacer" then
        spellInfo = spellName;
    end

    if spellInfo:endswith("remaining") then
        timeRemainingString = spellInfo;
        spellInfo = "-";
    end

    if timeRemainingString then
        local time, timeType = string.match(timeRemainingString, "(%d+) (%w+) remaining");
        if time and timeType and timeType:contains("hour") then
            timeLeft = tonumber(time) * 60 * 60;
        elseif time and timeType and timeType:contains("minute") then
            timeLeft = tonumber(time) * 60;
        elseif time and timeType and timeType:contains("second") then
            timeLeft = tonumber(time);     
        end 
    end    
    

    return spellName, spellInfo, buffType, timeLeft;
end
...
local function RegenerateBuffList(unit, dstGUID)
    ...
    for i=1, 32 do
        ...
        if spellName then
            ...
            local _, spellInfo, buffType, timeRemaining = getToolTipInfo(scanTip);
            if spellID then
                local icon = GetSpellTexture(spellID)
                local opts = spells[spellID]
                local buffInfo = { spellName, icon, 0, (opts and opts.buffType), 0, 0, nil, nil, nil, spellID, false, false, false, false, 1, false }
                ...
                -- get accurate duration via combatlog or accurate tooltip timers
                if duration then
                    buffInfo[5] = duration
                    buffInfo[6] = expirationTime
                -- fallback, read timer from tooltip for KNOWN buffs 
                elseif opts and opts.duration and timeRemaining then
                    local cleanDuration = cleanDuration(opts.duration, spellID, srcGUID, 0)
                    buffInfo[5] = cleanDuration
                    buffInfo[6] = timeRemaining + GetTime()
                -- backup, read timer from tooltip for unknown buffs (not in list) that the player has encountered int he world before
                elseif not duration then
                    local spellNameKey = spellName .. "_" .. spellInfo .. "_" .. buffType;
                    local storeSpellInfo = LCD_SpellInfo[spellNameKey];
                    if storeSpellInfo and storeSpellInfo.maxDuration and timeRemaining then
                        buffInfo[5] = storeSpellInfo.maxDuration
                        buffInfo[6] = timeRemaining + GetTime()
                    end
                end
                ...
                -- we couldn't find a spellID, but some buffs exist regardless
                -- those buffs can be found while journeying through the world
                -- if the tooltip can read them, we should make them available
                local spellNameKey = spellName .. "_" .. spellInfo .. "_" .. buffType;
                local storeSpellInfo = LCD_SpellInfo[spellNameKey];

                if storeSpellInfo then
                    spellID = storeSpellInfo.spellID
                    local icon = storeSpellInfo.texture
                    local buffInfo = { spellName, icon, 0, (storeSpellInfo.buffType), 0, 0, nil, nil, nil, spellID, false, false, false, false, 1, false }
                    local isStacking = storeSpellInfo.stacking
                    local srcGUID = nil
                    local duration, expirationTime = GetGUIDAuraTime(dstGUID, spellName, spellID, srcGUID, isStacking)
                    -- get accurate duration via combatlog or accurate tooltip timers
                    if duration then
                    ...
                    -- fallback, read timer from tooltip for KNOWN buffs
                    if (not duration or duration == 0) and storeSpellInfo.maxDuration and timeRemaining then
                        buffInfo[5] = storeSpellInfo.maxDuration
                        buffInfo[6] = timeRemaining + GetTime()                        
                    end
...
function updateSpellInfoStore(unit, index, filter)
    -- store some information to be recalled later
    if filter == "HELPFUL" then
        local buffName, icon, count, debuffType, duration, expirationTime, caster, canStealOrPurge, nps, spellId = UnitAura(unit, index, filter);
        if not buffName then return end

        scanTip:ClearLines()
        scanTip:SetUnitAura(unit, index, "HELPFUL")

        local spellName, spellInfo, buffType = getToolTipInfo(scanTip);

        local spellNameKey = buffName .. "_" .. spellInfo .. "_" .. buffType;

        if not buffName == spellName then
            print("[LCD]: There was an issue! Tooltip of buff " .. spellName.. " does not match UnitAura " .. buffName)
            return;
        end

        -- no need to update
        if LCD_SpellInfo[spellNameKey] then
            return;
        end

        --print("[LCD]: Found new buff " .. spellName.. "(" .. spellId .. "): " .. spellInfo .. " - storing data.")

        LCD_SpellInfo[spellNameKey] = {
            ["spellID"] = spellId,
            ["texture"] = icon,
            ["buffType"] = buffType,
            ["maxDuration"] = duration,
            ["stacking"] = count and count > 0,
        };
    end
end

-- This whole block updates timers on target via tooltips frequently
-- If a timer changes within the last 0.1 seconds, it's considered precise  - we then stop updating it UNLESS it ends up being refreshed
-- Timers that are already available WILL be ignored
-- To save resources, we only do this every 100ms for "target"
local targetTip = CreateFrame("GameTooltip", "LibClassicDurationsTargetTip", nil, "GameTooltipTemplate")
targetTip:SetOwner(WorldFrame, "ANCHOR_NONE")

local tooltipTimers = {};

function updatedTargetBuffTimers()
    local guid = UnitGUID("target");
    if guid and not UnitIsFriend("player", "target") then
        for i=1, 32 do
            if not tooltipTimers[guid] then tooltipTimers[guid] = {} end
            targetTip:ClearLines();
            targetTip:SetUnitAura("target", i, "HELPFUL");
            local spellName, spellInfo, buffType, timeRemaining = getToolTipInfo(targetTip);
            if not spellName then break end

            local fromDB = true;
            local fromSearch = false;
            local spellID = GetLastRankSpellID(spellName);
            local storeSpellInfo = nil;

            if not spellID then
                fromDB = false;
                local spellNameKey = spellName .. "_" .. spellInfo .. "_" .. buffType;
                storeSpellInfo = LCD_SpellInfo[spellNameKey];
                if storeSpellInfo then
                    fromSearch = true;
                    spellID = storeSpellInfo.spellID;
                end
            ...
            -- only do this for spells which are known one way or another AND are known to ahve a timer
            if spellID and timeRemaining and (fromDB or fromSearch) then
                local existingTimers = tooltipTimers[guid][spellID]
                -- timer just switched to a different number in the last 0.1 seconds, we are now "accurate"
                -- we cannot just check for lower, because the timer can ALSO be refreshed to a larger number, think: hots
                if existingTimers and not existingTimers.accurate and existingTimers.timeRemaining ~= timeRemaining then
                    -- FIXME: If maxDuration is determined by this function, store it and restore it somehow?                 
                    local opts = (fromDB and spells[spellID] or { ["duration"] = npc_spells[spellID] }) or (fromSearch and { ["duration"] = storeSpellInfo.maxDuration, ["buffType"] = storeSpellInfo.buffType } or {});
                    local cleanDuration = cleanDuration(opts.duration, spellID, srcGUID, 0)
                    if not cleanDuration then
                        --print("Missing duration for", "opts", spells[spellID] ~= nil, "storeSpellInfo", storeSpellInfo ~= nil, spellID)
                        if timeRemaining <= 60 then
                            cleanDuration = timeRemaining + 1;
                        else
                            -- set max duration to one hour as a fallback
                            -- what really matters is the time left, not the theoretical max
                            cleanDuration =  3600;
                        end    
                    end
                    local destFlags = COMBATLOG_OBJECT_REACTION_HOSTILE + COMBATLOG_OBJECT_CONTROL_PLAYER + COMBATLOG_OBJECT_TYPE_PLAYER + COMBATLOG_OBJECT_TARGET

                    local spellTable = SetTimer(nil, guid, UnitName("target"), destFlags, spellID, spellName, opts, opts.buffType, false)

                    if spellTable then
                        -- set startTime, calculated backwards from "accurate" timeLeft
                        spellTable[2] = GetTime() - cleanDuration + timeRemaining;
                        existingTimers.accurate = true;

                        -- clear cache
                        buffCacheValid[guid] = nil;
                        buffCache[guid] = nil;
                        -- fire event to force addons to refresh
                        FireToUnits("UNIT_BUFF", guid)
                        --print("Set timer for " .. spellName .. " to " .. timeRemaining)
                        -- fire event, when this buff expires - all these events need to be simulated because (obviously) UNIT_AURA does not fire for enemies since buffs are not supposed to be visible
                        -- combat log does catch some but not ALL of these events
                        -- FIXME: remember events and don't fire if the same was already removed (and an event fired) in the last 0.5 sec or so, to prevent spam
                        -- FIXME: abort timer if spell gets refreshed in ifelse underneath
                        C_Timer.After(timeRemaining + 0.1, function()
                            -- remove buff and all timers
                            SetTimer(nil, guid, UnitName("target"), destFlags, spellID, spellName, opts, opts.buffType, true);
                            -- clear cache
                            buffCacheValid[guid] = nil;
                            buffCache[guid] = nil;
                            FireToUnits("UNIT_BUFF", guid)
                ...
                -- the duration may have changed via a timer refresh, we need to reset accuracy    
                elseif existingTimers and existingTimers.accurate and existingTimers.timeRemaining < timeRemaining then
                    existingTimers.accurate = false;
                    --print("Reset timer for " .. spellName .. " to " .. timeRemaining)
                end

                if existingTimers then
                    existingTimers.lastTimeRemaining = existingTimers.timeRemaining;
                    existingTimers.timeRemaining = timeRemaining;
                else
                    tooltipTimers[guid][spellID] = {
                        ["spellName"] = spellName,
...
C_Timer.NewTicker(0.1, updatedTargetBuffTimers)

function lib.UnitAuraDirect(unit, index, filter)

    updateSpellInfoStore(unit, index, filter);
    ...
    if enableEnemyBuffTracking and filter == "HELPFUL" and not UnitIsFriend("player", unit) and not UnitAura(unit, 1, filter) then
        ...
        -- cache is valid for roughly 40 seconds, it can get invalidated if an update of timers is required
